...
impl VulkanExample {
    ...
    pub fn new(initializer: AssetInitializer) -> GsResult<VulkanExample> {
        ...
        let (model_entity, model_storage, ubo_buffer, ubo_storage) = {
        ...
        let pipelines = {
            VulkanExample::pipelines(&initializer, &model_entity, &ubo_set, &depth_attachment, screen_dimension)
        }?;

        let present_availables = {
            VulkanExample::sync_resources(&initializer, &pipelines.pipeline_set)
        }?;

        let (command_pool, command_buffers) = {
            VulkanExample::commands(&initializer, &pipelines, &model_entity, &ubo_set)
        }?;
    ...
    fn pipelines(initializer: &AssetInitializer, model_entity: &GsglTFEntity, ubo_set: &DescriptorSet, depth_image: &GsDSAttachment, dimension: vkDim2D) -> GsResult<PipelineContent> {

        // shaders ------------------------------------------------------------------------
        let vertex_input_desc = Vertex::input_description();

        // shaders of Phone Pipeline.
        let phong_vertex_shader = GsShaderCI::from_source(GsPipelineStage::VERTEX, Path::new(PHONG_VERTEX_SHADER_SOURCE_PATH), None, "[Phone Vertex Shader]");
        let phone_fragment_shader = GsShaderCI::from_source(GsPipelineStage::FRAGMENT, Path::new(PHONG_FRAGMENT_SHADER_SOURCE_PATH), None, "[Phone Fragment Shader]");
        let phong_shader_infos = vec![phong_vertex_shader, phone_fragment_shader];

        // shaders of Toon Pipeline.
        let tone_vertex_shader = GsShaderCI::from_source(GsPipelineStage::VERTEX, Path::new(TOON_VERTEX_SHADER_SOURCE_PATH), None, "[Toon Vertex Shader]");
        let tone_fragment_shader = GsShaderCI::from_source(GsPipelineStage::FRAGMENT, Path::new(TOON_FRAGMENT_SHADER_SOURCE_PATH), None, "[Toon Fragment Shader]");
        let tone_shader_infos = vec![tone_vertex_shader, tone_fragment_shader];

        // shaders of Wireframe Pipeline.
        let wireframe_vertex_shader = GsShaderCI::from_source(GsPipelineStage::VERTEX, Path::new(WIREFRAME_VERTEX_SHADER_SOURCE_PATH), None, "[Wireframe Vertex Shader]");
        let wireframe_fragment_shader = GsShaderCI::from_source(GsPipelineStage::FRAGMENT, Path::new(WIREFRAME_FRAGMENT_SHADER_SOURCE_PATH), None, "[Wireframe Fragment Shader]");
        ...
        let pipeline_template = GfxPipelineConfig::new(phong_shader_infos, vertex_input_desc, render_pass, initializer.screen_dimension())
        ...
        let mut pipeline_builder = GfxPipelineSetBuilder::new(initializer, pipeline_template)?;
    ...
    fn sync_resources(initializer: &AssetInitializer, pipelines: &GsPipelineSet<Graphics>) -> GsResult<Vec<GsSemaphore>> {

        // sync
        let mut present_availables = Vec::with_capacity(pipelines.frame_count());
        for _ in 0..pipelines.frame_count() {
    ...
    fn commands(initializer: &AssetInitializer, pipelines: &PipelineContent, model_entity: &GsglTFEntity, ubo_set: &DescriptorSet) -> GsResult<(GsCommandPool, Vec<GsCommandBuffer>)> {
        ...
        for (frame_index, command) in raw_commands.into_iter().enumerate() {
            ...
            let mut recorder = GsCmdRecorder::<Graphics>::new(initializer, &phong_pipeline, command);
...
impl GraphicsRoutine for VulkanExample {
    ...
    fn reload_res(&mut self, initializer: AssetInitializer) -> GsResult<()> {
        ...
        self.pipelines = VulkanExample::pipelines(&initializer, &self.model_entity, &self.ubo_set, &self.depth_attachment, initializer.screen_dimension())?;

        self.present_availables = VulkanExample::sync_resources(&initializer, &self.pipelines.pipeline_set)?;

        let (command_pool, command_buffers) = VulkanExample::commands(&initializer, &self.pipelines, &self.model_entity, &self.ubo_set)?;
