...
impl VulkanExample {
    ...
    pub fn new(loader: AssetsLoader) -> GsResult<VulkanExample> {
        ...
        let (vertex_buffer, index_buffer, vertex_storage, ubo_view_buffer, ubo_instance_buffer, ubo_storage) = loader.assets(|kit| {
            VulkanExample::buffers(kit, &ubo_data_dynamic)
        })?;
        ...
        let (ubo_set, desc_storage) = loader.assets(|kit| {
            VulkanExample::ubo(kit, &ubo_view_buffer, &ubo_instance_buffer)
        })?;
        ...
        let (command_pool, command_buffers) = loader.commands(|kit| {
            VulkanExample::commands(kit, &pipeline, &vertex_buffer, &index_buffer, &ubo_set, dynamic_alignment as _, &view_port, &scissor)
        })?;
    ...
    fn buffers(kit: AllocatorKit, ubo_instance_data: &UboDataDynamic) -> GsResult<(GsVertexBuffer, GsIndexBuffer, GsBufferRepository<Device>, GsUniformBuffer, GsUniformBuffer, GsBufferRepository<Host>)> {
        ...
        let ubo_view_info = GsBufUniformInfo::new(0, 1, data_size!(UboView));
        let ubo_view_index = ubo_allocator.assign(ubo_view_info)?;

        // refer to `layout (set = 0, binding = 1) uniform UboInstance` in base.vert.
        let ubo_instance_info = GsBufUniformInfo::new_dyn(1, 1, data_size!(Matrix4F), OBJECT_INSTANCE);
    ...
    fn ubo(kit: AllocatorKit, ubo_view_buffer: &GsUniformBuffer, ubo_instance_buffer: &GsUniformBuffer) -> GsResult<(DescriptorSet, GsDescriptorRepository)> {
    ...
    fn commands(kit: CommandKit, graphics_pipeline: &GsPipeline<Graphics>, vertex_buffer: &GsVertexBuffer, index_buffer: &GsIndexBuffer, ubo_set: &DescriptorSet, dynamic_alignment: vkuint, view_port: &CmdViewportInfo, scissor: &CmdScissorInfo) -> GsResult<(GsCommandPool, Vec<GsCommandBuffer>)> {
...
impl GraphicsRoutine for VulkanExample {
    ...
    fn reload_res(&mut self, loader: AssetsLoader) -> GsResult<()> {
        ...
        let (command_pool, command_buffers) = loader.commands(|kit| {
            VulkanExample::commands(kit, &self.pipeline, &self.vertex_buffer, &self.index_buffer, &self.ubo_set,  self.dynamic_alignment as _, &self.view_port, &self.scissor)
        })?;
