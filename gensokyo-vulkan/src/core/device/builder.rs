...
impl<'a> LogicalDeviceBuilder<'a> {
    ...
    pub fn build(&mut self) -> Result<GsLogicalDevice, LogicalDeviceError> {
        ...
        let device = GsLogicalDevice::new(handle, queue_container, graphics_queue, present_queue, transfer_queue);
        Ok(device)
    }
}
...
impl QueueRequestInfo {
    ...
    fn new(physical: &GsPhysicalDevice, strategy: QueueRequestStrategy) -> Result<QueueRequestInfo, LogicalDeviceError> {
        ...
        match self {
            | QueueRequestInfo::Queue { entity, logics } => {
                let unique_queue = unsafe {
                    ...
                    GsQueue::new(handle, entity.priority, entity.family_index, entity.queue_index)
                };
            ...
            | QueueRequestInfo::Queues { entities, logics, .. } => {
                let multiqueues = entities.iter()
                    .map(|entity_queue_info| {
                        ...
                        let queue = GsQueue::new(handle, entity_queue_info.priority, entity_queue_info.family_index, entity_queue_info.queue_index);
                        Rc::new(queue)
                    }).collect();
...
fn collect_to_container(device: &ash::Device, logic_queues: &Vec<LogicQueueInfo>, config: &DeviceConfig, unique_queue: Option<&Rc<GsQueue>>, multi_queues: Option<&Vec<Rc<GsQueue>>>) ->  Result<QueueContainer, LogicalDeviceError> {
